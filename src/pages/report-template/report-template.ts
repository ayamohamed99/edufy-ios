...
export class ReportTemplatePage{
  ...
  getTemplate(itm) {
    ...
    if (itm.dailyReportQuestionType.title == 'SINGLE_SHORT_TEXT_ONE_VIEW_SELECTED' ) {
      let label = "";
      let row = "";
      let radioList = "";
      let textList = "";
      ...
      for (let i = 0; i < itm.parametersList.length; i++) {
        if (itm.parametersList[i].key == "OPTION_HELPER_TITLE") {
          ...
          if (helpertext) {
            ...
            textList = "";
            countRow++;
            textList += '<section class="col-md-5" style="margin-left: 14px;"><label class="" style="">' + itm.parametersList[i].value + '</label><label class="input"><input type="text" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']" ng-change="override()"></label></section>'
            countParameters++;
            helpertext = false;
            count = 0;
          }
          ...
          if (count != switchToHelper) {
            helpertext = true;
            radioList += '<label class="radio"><input type="radio" name="radio-inline' + itm.questionNumber + countRow + '" value="' + itm.parametersList[i].value + '" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']" ng-change="override()"><i></i>' + itm.parametersList[i].value + '</label>';
            count++;
          }
          else {
            helpertext = true;
      ...
      for (let i = 0; i < itm.parametersList.length; i++) {
        if (itm.parametersList[i].key == "OPTION_HELPER_TITLE") {
          ...
          if (helpertext) {
            ...
            textList = "";
            countRow++;
            textList += '<section class="col-md-3" style="margin-left: 14px;"><label class="" style="">' + itm.parametersList[i].value + '</label><label class="input"><input type="text" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']" ng-change="override()"></label></section>'
            countParameters++;
            helpertext = false;
            count = 0;
          }
      ...
      this.reportTemplate +=  '<div class="formLabel" style="width:100%;">' + itm.question +
        ':<button type="button" class="btn btn-outline-warning btn-sm" ng-if="dailyReportQuestions[' + itm.questionNumber + '].editQuestion" ng-click="cancelEditigQuestion(' + itm.questionNumber + ')"  style="float:right; margin-left: 7px;" ><i style="font-size:15px; color: #3B9FF3;"  >Cancel </i></button><button type="button" class="btn btn-outline-warning btn-sm" ng-if="editQuestionAllowed" ng-click="editSaveQuestion(' + itm.questionNumber + ')" style="float:right; margin-right: 8px;" >' +
        ...
        '" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[parameter.id]"><i></i>{{parameter.value}} <a style="font-size:20px; color:firebrick; float:right;" ng-if="dailyReportQuestions[' + itm.questionNumber + '].editQuestion"><span class="btn btn-link-danger glyphicon glyphicon-remove" ng-click="removeQuestionParameter(' + itm.questionNumber + ',parameter.id)"></span></a></label></div>' +
        '</div><div style="width:100%;" ng-if="dailyReportQuestions[' + itm.questionNumber + '].editQuestion" class="input-group"><div><label class="formLabel"><i></i><span></span></label></div><div class="input-group"><input type="text" ng-model="dailyReportQuestionsEditParamTemps[' + itm.questionNumber + '].parameters[0].value" class="form-control" placeholder=" Pleas enter your new question" /><span class="input-group-btn" style="padding-left:10px;"><button class="btn btn-secondary btn-sm" ng-click="addParameterForQuestion(' + itm.questionNumber + ')"><span class="glyphicon glyphicon-plus" style="font-size:15px; color: #4CAF50;"></span><i style="font-size:15px; color: #3B9FF3;"> Add<i></button></span></div>'
        + '</div>';
    }

    else if (itm.dailyReportQuestionType.title == 'LONG_TEXT_MULTISELECT_VIEW_SELECTED_ONE_ANSWER' ){
      let radioList = "";
      for (let i = 0; i < itm.parametersList.length; i++) {
        radioList += '<label class="radio"><input type="radio" name="radio-inline' + itm.questionNumber + '" value="' + itm.parametersList[i].value + '" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer"><i></i>' + itm.parametersList[i].value + '</label>';
      }
      this.reportTemplate += '<label class="formLabel">' + itm.question + ':</label><div class="row"><div class="col col-12">' + radioList + '</div></div>';
    }

    else if (itm.dailyReportQuestionType.title == 'CONSTANT_SHORT_HELPER_TEXT_QUESTION' ){
      let textList = "";
      for (let i = 0; i < itm.parametersList.length; i++) {
        textList += '<section class="col col-2" style="margin-left: 9px;"><label class="formLabel ng-scope" style="margin-top: 5px;">' + itm.parametersList[i].value + '</label><label class="input"><input type="text" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer.' + i + '"></label></section>';
      }
      this.reportTemplate += '<label class="formLabel">' + itm.question + ':</label><div class="row" style="">' + textList + '</div>';
    }

    else if (itm.dailyReportQuestionType.title == 'SHORT_HELPER_TEXT_QUESTION' ){
      let textList = "";
      for (let i = 0; i < itm.parametersList.length; i++) {
        textList += '<section class="col col-2" style="margin-left: 9px;"><label class="formLabel ng-scope" style="margin-top: 5px;">' + itm.parametersList[i].value + ':</label><label class="input"><input type="text" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer.' + i + '"></label></section>';
      }
    ...
    else if (itm.dailyReportQuestionType.title == 'DROPDOWN_MENU_ONE_VIEW_SELECTED_EN' ||
      itm.dailyReportQuestionType.title == 'DROPDOWN_MENU_ONE_VIEW_SELECTED_AR'){
      let label = "";
      let row = "";
      let radioList = "";
      let textList = "";
      ...
      for (let i = 0; i < itm.parametersList.length; i++) {
        ...
        else if (itm.parametersList[i].key == "OPTION_DROP_DOWN") {
          optionHelperText++;
      ...
      for (let i = 0; i < itm.parametersList.length; i++) {
        if (itm.parametersList[i].key == "OPTION_HELPER_TITLE") {
          ...
          if (helpertext) {
            ...
            textList = "";
            countRow++;
            textList += '<section class="col-md-12" style="margin-left: 14px;"><label class="" style="width=100%">' + itm.parametersList[i].value + '</label>' +
              ' <ui-select ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']" theme="bootstrap" style="width:100%;"  ng-click="selectAction(' + itm.id + ')">' +
          ...
          if (count != switchToHelper) {
            helpertext = true;
            radioList += '<label class="radio" style="margin-right: 0px;margin-left: 35px;"><input type="radio" name="radio-inline' + itm.questionNumber + countRow + '" value="' + itm.parametersList[i].value + '" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']"><i></i>' + itm.parametersList[i].value + '</label>';
            count++;
          }
          else {
            helpertext = true;
            radioList += '<label class="radio" style="margin-right: 0px;margin-left: 35px;"><input type="radio" name="radio-inline' + countRow + '" value="' + itm.parametersList[i].value + '" ng-model="dailyReportAnswer.dailyReportAnswersObjectsList[' + itm.questionNumber + '].answer[' + countParameters + ']"><i></i>' + itm.parametersList[i].value + '</label> </div>';
            countParameters++;
      ...
      console.info("ThigetTemplates type not mapped: " + itm.dailyReportQuestionType.title);
      this.reportTemplate += '<label class="formLabel">' + itm.question + ':</label><div class="row"><div class="col col-4"></div></div>';

    }
    }

  enableOther = function (questionN, i) {
    let question = this.dailyReportQuestions[questionN];
