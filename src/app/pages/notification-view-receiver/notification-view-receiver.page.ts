...
export class NotificationViewReceiverPage implements OnInit {
  ...
  constructor(
    ...
    public toastCtrl: ToastViewService,
  ...
  getItems(ev: any) {
    ...
    this.receiverListStudents = this.originalReceiverListStudents.map(
      (value) => {
        return { ...value };
      }
    );
  ...
  filterData(val, index, studentName, studentIndex) {
    if (studentName.toLowerCase().indexOf(val.toLowerCase()) > -1) {
      let student = this.originalReceiverListStudents[index]["studentlist"][
        studentIndex
      ];
      this.receiverListStudents[index]["studentlist"].push(
        this.originalReceiverListStudents[index]["studentlist"][studentIndex]
      );
    }
  }
  ...
  getReceivers() {
    ...
    this.notificationServ
      .getRecieverList(this.notifications.notificationId)
      .subscribe(
        // @ts-ignore
        (response) => {
          let Data = response;
          ...
          if (this.originalReceiverListStudents.length == 0) {
            ...
            let intervaldata = setInterval(() => {
              ...
              for (
                let i = 0;
                i < this.originalReceiverListStudents.length;
                i++
              ) {
                let num = this.originalReceiverListStudents[i]["studentlist"]
                  .length;
                for (let j = 0; j < num; j++) {
                  if (
                    this.originalReceiverListStudents[i]["studentlist"][j]
                      .seenByParent ||
                    this.originalReceiverListStudents[i]["studentlist"][j]
                      .seenByStudent
                  ) {
              ...
              this.TotalNumOfReceivers =
                this.SeenNumOfReceivers + this.UnseenNumOfReceivers;
                    ...
                    datasets: [
                      {
                        label: "# of Votes",
                        data: [
                          this.SeenNumOfReceivers,
                          this.UnseenNumOfReceivers,
                        ],
                        backgroundColor: ["#1AC61F", "#EC1B23"],
                        hoverBackgroundColor: ["#1AC61F", "#EC1B23"],
                      },
                    ],
                  },
                }
  ...
  createReceiversPdf() {
    ...
    for (let i = 0; i < this.tempRList.length; i++) {
      ...
      for (let j = 0; j < receiverObject.List.length; j++) {
        if (
          receiverObject.List[j].seenByParent ||
          receiverObject.List[j].seenByStudent
        ) {
  ...
  downloadPDF(pdf) {
    ...
    if (this.platform.is("cordova")) {
      ...
      let utf8 = new Uint8Array(
        this.convertDataURIToBinary(pdf.output("datauristring"))
      );
      ...
      that.file
        .writeFile(storageDirectory, fileName, blob, { replace: true })
        .then((fileEntry) => {
          console.log(fileEntry);
          that.fileOpener
            .open(storageDirectory + fileName, "application/pdf")
