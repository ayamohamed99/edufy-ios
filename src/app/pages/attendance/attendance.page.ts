...
export class AttendancePage implements OnInit {
  ...
  constructor(
    ...
    public attendServ: AttendanceTeachersService,
    public tranDate: TransFormDateService,
    ...
    this.storage
      .get(this.localStorageAttendanceWIFIS)
      .then(
        (value) => {
          let date = JSON.parse(value);
          date.forEach((val) => {
        ...
        (value) => {
          ...
          if (this.userShiftData != null) {
            if (
              ...
              if (
                this.userShiftData.checkOutDate != null &&
                this.accountServ.getUserRole().checkInAttendance
              ) {
                this.checkIn = "CheckIn";
              } else if (
  ...
  checktheDateIn(date) {
    let savedShiftDate = this.tranDate.transformTheDate(
      new Date(date),
      "yyyy/MM/dd"
    );
  ...
  funcCheckIn() {
    if (this.network.type != "wifi") {
      ...
      this.attendServ
        .checkInAttendance(date, this.accountServ.userId)
        .subscribe(
          (value) => {
            this.tempData = value;
  ...
  checkInCondition() {
    if (
      ...
      (this.userShiftData == null || this.userShiftData.checkOutDate != null)
    ) {
      return true;
    } else {
      return false;
  ...
  async presentChangePhoneAlert() {
    const alert = await this.alertController.create({
      header: "Alert",
      message:
        "This phone not connect to your account do you want to change your phone and inform your admin?",
      buttons: [
        {
          text: "Cancel",
          role: "cancel",
          ...
          .subscribe(
            (value) => {
              ...
              this.load.stopLoading().then((val) => {
                ...
                this.todayAttendTempNull.forEach((val, index) => {
                  this.todayAttend.push({
                    checkout_DATE: val.checkout_DATE,
                    checkin_DATE: val.checkin_DATE,
                    shift_PENALTY_id: val.shift_PENALTY_id,
  ...
  getAttendanceWeek() {
    ...
    if (!this.accountServ.getUserRole().attendanceAllTeachersAppear) {
      ...
      from = this.tranDate.getStartDateOfWeek(new Date(), "yyyy-MM-dd HH:MM");
      let endDate = this.tranDate.getEndDateOfWeek(
        new Date(),
        "yyyy-MM-dd HH:MM"
      );
    ...
    this.load
      ...
      .then((value) => {
        this.attendServ
          .getAllUserAttendanceTodayOrWeek(branchId, userId, from, to, false)
          .subscribe(
            (value) => {
              // @ts-ignore
              let allData: [any] = value;
              this.load.stopLoading().then((val) => {
                ...
                this.weeklyAttend = [];
                if (
                  this.accountServ.getUserRole().attendanceAllTeachersAppear
                ) {
                ...
                allData.forEach((val, index) => {
                  if (this.selectedTab == this.TODAY_TAB) {
                    ...
                    if (
                      !this.accountServ.getUserRole()
                        .attendanceAllTeachersAppear
                    ) {
                      if (val.user.name) {
                ...
                if (
                  ...
                  this.weeklyAttendTempNull.forEach((val, index) => {
                    this.weeklyAttend.push({
                      checkInDate: val.checkin_DATE,
                      checkOutDate: val.checkout_DATE,
                      penaltyRules: val.shift_PENALTY_id,
  ...
  loadData(ev) {
    let branchId = this.accountServ.userBranchId;
    let from = this.tranDate.transformTheDate(
      this.weekDays[this.nextDayIndexInArray],
      "yyyy-MM-dd HH:MM"
    );

    this.attendServ
      .getAllUserAttendanceTodayOrWeek(branchId, null, from, null, false)
      .subscribe(
        (value) => {
          // @ts-ignore
          let allData: [any] = value;
  ...
  setDateFormat(date) {
    let newFormat = this.tranDate.transformTheDate(
      new Date(date),
      "dd / MMM / yyyy"
    );
    return newFormat;
