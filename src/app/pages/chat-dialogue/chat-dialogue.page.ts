...
export class ChatDialoguePage implements OnInit {
  @ViewChild("ChatDialogue", { static: false }) private ChatDialogueScroll: any;
  student;
  chatDialogs: any[] = [];
  theMessage;
  loading = false;
  ...
  getChatHistory() {
    this.loading = true;
    this.chatServ
      .getChatMessagesHistory(this.student.id, this.student.classes.branch.id)
      .subscribe(
        // @ts-ignore
        (val) => {
          this.loading = false;
          ...
          for (let message of data) {
            ...
            chat.chatMessageRecieverStatesList =
              message.chatMessageRecieverStatesList;
        ...
        (error1) => {
          this.loading = false;
          this.tost.presentPositionToast(
            "Oops! I'm sorry. can't get your chat history",
            "bottom"
          );
        }
      );
  ...
  sendMessage() {
    ...
    chat.user = {
      id: this.accountServ.userId,
      name: this.accountServ.getUserName(),
    };
    this.chatDialogs.push(chat);
    this.scrollDown();
    this.chatServ
      .sendChat(
        this.student.id,
        this.theMessage,
        this.accountServ.userId,
        this.accountServ.getUserName()
      )
      .subscribe(
        // @ts-ignore
        (val) => {
        ...
        (error1) => {
          ...
          this.sendLoading = false;
          this.tost.presentPositionToast(
            "Oops! I'm sorry. can't send the message",
            "bottom"
          );
        }
      );
  ...
  reloadSend(data, index) {
    document
      .getElementById("errorIcon" + index)
      .classList.toggle("rotate-scale-up");
    this.chatServ
      .sendChat(
        this.student.id,
        data.message,
      ...
      .subscribe(
        ...
        (val) => {
          ...
          //     message = JSON.parse(val.data)
          // }
          this.chatDialogs[index].chatMessageRecieverStatesList =
            message.chatMessageRecieverStatesList;
