...
export class NotificationPage implements OnInit {
  ...
  constructor(
    ...
    private transferF: FileTransfer,
    ...
    popoverOption.onDidDismiss().then((val) => {
      if (val == null) {
        ...
        if (data.done === "deleteSuccess") {
          ...
          this.getNotificationReciver(
            notification.notificationId,
            notification.title,
            notification.body,
            notification.recreceiversList,
            notification.tagsListName,
            i
          );
        }
      }
  ...
  getAllClasses() {
    this.classesServ
      .getClassList("NOTIFICATION", 2, null, null, null, null)
      .subscribe(
        (value) => {
  ...
  async presentEditAsNewNotificationModal() {
    this.load.stopLoading().then(async (value) => {
      let data = {
        ...
        studetsNameList: this.studentsName,
        studentsdetailsList: this.studentwithClass,
        recieverList: this.reciversList,
        tagList: this.editTags,
        attachmentList: this.editAsNewAttachmentList,
      };
  ...
  async onAttachmentClick(
    ...
    if (
      pending != "pending" &&
      this.accountServ.getUserRole().notificationAttachmentDownload
    ) {
      this.alrtCtrl
        .create({
          header: "Attachment",
  ...
  OpenFiles(
    ...
    transfer
      .download(attachmentURL, path + attachmentName)
      .then((entry) => {
        this.load.stopLoading().then(() => {
          let url = entry.toURL();
            ...
            .catch((e) => {
              this.load.stopLoading().then(() => {
                ...
                if (e.message.includes("Activity not found")) {
                  ...
                  error =
                    "Something went wrong try again later." + JSON.stringify(e);
                }
  ...
  downloadFile(
    ...
    this.platform.ready().then(() => {
      ...
      fileTransfer
        .download(fileLocation, storageDirectory + attachmentName)
        .then(
          (entry) => {
            this.load.stopLoading().then(() => {
              this.presentAlertWithOk(
                `Download Success`,
                `${attachmentName} was successfully downloaded.`
              );
            });
          },
          (error) => {
            this.presentAlertWithOk(
              `Download Failed!`,
              `${attachmentName} was not successfully downloaded. Error code: ${error.code}`
            );
          }
        );
    });
    ...
    this.load.stopLoading().then((value) => {
      that.load.startLoading("", true, "loadingWithoutBackground").then(() => {
        if (that.platform.is("desktop")) {
          ...
          that.studentService.putHeader(
            localStorage.getItem(this.localStorageToken)
          );
          that.classesServ.putHeader(
            localStorage.getItem(this.localStorageToken)
          );
          that.getAllClasses();
        } else {
  ...
  approveNotification(index) {
    ...
    if (approvedNotification.fbPosted) {
      // if need
      if (approvedNotification.attachment == null) {
        // check
        ...
        this.notificationService
          .editNotification(sentNotification, 2)
          .subscribe(
            //@ts-ignore
            (response) => {
              // this.loading.dismiss();
              ...
              this.toastCtrl.presentPositionToast(
                "Notification approved & sent successfully.",
                "bottom"
              );
            },
            (reason) => {
              elButton.classList.remove("onclic");
              this.toastCtrl.presentPositionToast(
                "Problem approving notifications.",
                "bottom"
              );
              console.error(
                "Error: notification.module>NotificationCtrl>approveNotification> cannot send notification -  " +
                  reason
              );
            }
          );
