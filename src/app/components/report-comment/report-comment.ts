...
export class ReportCommentComponent implements OnInit, AfterViewChecked {
  ...
  async showErrorToast(operationName: string) {
  ...
  approveComment(index) {
    ...
    const comment = this.currentReportComments[index] ;
    this.commentsProvider.editComment(null, null,
      ...
      .subscribe(response => {
        ...
        this.isEditedCommentsLoading[comment.id] = false;
        this.inEditModeComments[comment.id] = false;
        const elItem = document.getElementById('itemView' + index);
        const elButton = document.getElementById('buttonView' + index);
